//
//  AKCompanyNoticeContentViewController.m
//  农化通
//
...
@interface AKCompanyNoticeContentViewController ()
...
@implementation AKCompanyNoticeContentViewController
...
- (void)addQueryView
{
    ...
    UIView *minQueryView = [AKFastView viewWithWithFrame:CGRectMake(20, 40, 280, 170) andBackgroundColor:[UIColor blackColor]];
    ...
    UILabel *topLabel = [AKFastLabel labelWithFrame:CGRectMake(50, 0, 280, 44) andText:@"公告查询：" andtextAlignment:UITextAlignmentLeft andFont:22];
    topLabel.textColor = [UIColor whiteColor];
    UILabel *queryLabel1 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 50, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"开始时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel1.textColor = [UIColor whiteColor];
    UILabel *queryLabel2 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 65+_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"结束时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel2.textColor = [UIColor whiteColor];
    queryTextField1 = [AKFastTextField textFieldWithFrame:CGRectMake(95, 50, 150, 30) andBackgroundColor:[UIColor whiteColor] andBorderStyle:UITextBorderStyleBezel andKeyboardType:UIKeyboardTypeNumbersAndPunctuation andFont:15.f];
    queryTextField1.tag = 401;
    queryTextField1.delegate = self;
    queryTextField1.text = @"";
    queryTextField1.placeholder = @"格式：2013-1-1";
    queryTextField2 = [AKFastTextField textFieldWithFrame:CGRectMake(95, 90, 150, 30) andBackgroundColor:[UIColor whiteColor] andBorderStyle:UITextBorderStyleBezel andKeyboardType:UIKeyboardTypeNumbersAndPunctuation andFont:15.f];
    queryTextField2.tag = 402;
    queryTextField2.delegate = self;
    queryTextField2.text = @"";
    queryTextField2.placeholder = @"格式：2013-1-30";
    UIButton *queryBtn = [AKFastButton buttonWithType:UIButtonTypeCustom andFrame:CGRectMake(160, 80+2*_LINE_HEIGHT_, 80, 30) andNormalTitle:@"查询" andBackgroundImage:[UIImage imageNamed:@"button_60.png"] andTouchUpTarget:self andTouchUpAction:@selector(queryBegin)];
    [queryBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    UIButton *cancelBtn = [AKFastButton buttonWithType:UIButtonTypeCustom andFrame:CGRectMake(30, 80+2*_LINE_HEIGHT_, 80, 30) andNormalTitle:@"取消" andBackgroundImage:[UIImage imageNamed:@"button_60.png"] andTouchUpTarget:self andTouchUpAction:@selector(queryCancel)];
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"公司公告" andtextAlignment:UITextAlignmentCenter andFont:18];
...
{
    mum.hidden = NO;
    NSDictionary *aParameterDic = [NSDictionary dictionaryWithObjectsAndKeys:userData.userId,@"cid",userData.userAccount,@"mobile",userData.userPassword,@"pwd",@"list",@"action",@"",@"pagenum",@"",@"sdate",@"",@"edate", nil];
...
{
    [self touchesBegan:nil withEvent:nil];
    [parameterDic setObject:queryTextField1.text forKey:@"sdate"];
    [parameterDic setObject:queryTextField2.text forKey:@"edate"];
...
-(void)refreshSucceedWithContentDictionary:(NSDictionary*)aContentDic
{
    ...
    if ([flagStr isEqualToString:@"success"]) {
        NSArray *listArray = [aContentDic objectForKey:@"notice"];
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView companyNoticeCellViewWithArray:listArray]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AKNoticeDetailsViewController *noticeDetailsVC = [[AKNoticeDetailsViewController alloc]init];
    noticeDetailsVC.noticeId = idStr;
    [self.navigationController pushViewController:noticeDetailsVC animated:NO];
    [noticeDetailsVC release];
}
