//
//  AKActivityRecordContentViewController.m
//  农化通
//
...
@interface AKActivityRecordContentViewController ()
...
@implementation AKActivityRecordContentViewController
...
- (void)addQueryView
{
    ...
    UIView *minQueryView = [AKFastView viewWithWithFrame:CGRectMake(20, 40, 280, 300) andBackgroundColor:[UIColor blackColor]];
    ...
    UILabel *topLabel = [AKFastLabel labelWithFrame:CGRectMake(50, 0, 280, 44) andText:@"活动查询" andtextAlignment:UITextAlignmentLeft andFont:22];
    topLabel.textColor = [UIColor whiteColor];
    UILabel *queryLabel1 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 50, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"客户编码：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel1.textColor = [UIColor whiteColor];
    UILabel *queryLabel2 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 65+_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"客户名称：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel2.textColor = [UIColor whiteColor];
    UILabel *queryLabel3 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 80+2*_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@" 状   态 ：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel3.textColor = [UIColor whiteColor];
    UILabel *queryLabel4 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 95+3*_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"开始时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel4.textColor = [UIColor whiteColor];
    UILabel *queryLabel5 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 110+4*_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"结束时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    ...
    queryTextField4.placeholder = @"格式：2013-1-1";
    queryTextField4.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    queryTextField5 = [AKFastTextField textFieldWithFrame:CGRectMake(95, 210, 150, 30) andBackgroundColor:[UIColor whiteColor] andBorderStyle:UITextBorderStyleBezel andKeyboardType:UIKeyboardTypeDefault andFont:15.f];
    queryTextField5.tag = 405;
    queryTextField5.delegate = self;
    queryTextField5.text = @"";
    queryTextField5.placeholder = @"格式：2013-1-30";
    queryTextField5.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    UIButton *queryBtn = [AKFastButton buttonWithType:UIButtonTypeCustom andFrame:CGRectMake(160, 130+5*_LINE_HEIGHT_, 80, 30) andNormalTitle:@"查询" andBackgroundImage:[UIImage imageNamed:@"button_60.png"] andTouchUpTarget:self andTouchUpAction:@selector(queryBegin)];
    [queryBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    UIButton *cancelBtn = [AKFastButton buttonWithType:UIButtonTypeCustom andFrame:CGRectMake(30, 130+5*_LINE_HEIGHT_, 80, 30) andNormalTitle:@"取消" andBackgroundImage:[UIImage imageNamed:@"button_60.png"] andTouchUpTarget:self andTouchUpAction:@selector(queryCancel)];
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"活动记录" andtextAlignment:UITextAlignmentCenter andFont:18];
...
{
    mum.hidden = NO;
    NSDictionary *aParameterDic = [NSDictionary dictionaryWithObjectsAndKeys:userData.userId,@"cid",userData.userAccount,@"mobile",userData.userPassword,@"pwd",@"list",@"action",@"",@"pagenum",@"",@"dealerno",@"",@"dealername",@"",@"state",@"",@"sdate",@"",@"edate", nil];
    [parameterDic setDictionary:aParameterDic];
    [currentRequestContent currentRequestWithURL:_HUODONG_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
-(void)reportData//上报
{
    AKActivityReportViewController *activityReportVC = [[AKActivityReportViewController alloc]init];
    activityReportVC.delegate = self;
    [self.navigationController pushViewController:activityReportVC animated:NO];
    [activityReportVC release];
}
...
{
    [self touchesBegan:nil withEvent:nil];
    [parameterDic setObject:queryTextField1.text forKey:@"dealerno"];
    [parameterDic setObject:queryTextField2.text forKey:@"dealername"];
    [parameterDic setObject:queryTextField3.text forKey:@"state"];
    [parameterDic setObject:queryTextField4.text forKey:@"sdate"];
    [parameterDic setObject:queryTextField5.text forKey:@"edate"];
...
//    NSLog(@"%@",aContentDic);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        ...
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView activityRecordCellViewWithArray:listArray]];
