//
//  AKSendCargoRecordContentViewController.m
//  农化通
//
...
@interface AKSendCargoRecordContentViewController ()
-(void)ransmitRequest;//发送请求
-(void)ransmitRequestWithDic:(NSDictionary*)aDic;//发送请求
-(void)refreshContent;//刷新
@end

@implementation AKSendCargoRecordContentViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"退货记录" andtextAlignment:UITextAlignmentCenter andFont:18];
...
-(void)ransmitRequest//发送请求
{
    ...
    [currentRequestContent currentRequestWithURL:_TUIHUO_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
-(void)reportData//上报
{
    AKIndentReportViewController *sendCargoReportVC = [[AKIndentReportViewController alloc]init];
    sendCargoReportVC.delegate = self;
    sendCargoReportVC.reportTag = 3;
    sendCargoReportVC.reportName = @"退货上报";
    [self.navigationController pushViewController:sendCargoReportVC animated:NO];
    [sendCargoReportVC release];
}
...
//    NSLog(@"%@",aContentDic);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        ...
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView sendCargoRecordCellViewWithArray:listArray]];
