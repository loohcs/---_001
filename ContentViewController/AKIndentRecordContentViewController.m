//
//  AKIndentRecordContentViewController.m
//  农化通
//
...
@interface AKIndentRecordContentViewController ()
-(void)ransmitRequest;//发送请求
-(void)ransmitRequestWithDic:(NSDictionary*)aDic;//发送请求
-(void)refreshContent;//刷新
@end

@implementation AKIndentRecordContentViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"订单记录" andtextAlignment:UITextAlignmentCenter andFont:18];
...
-(void)ransmitRequest//发送请求
{
    ...
    [currentRequestContent currentRequestWithURL:_DINGDAN_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
-(void)reportData//上报
{
    AKIndentReportViewController *indentReportVC = [[AKIndentReportViewController alloc]init];
    indentReportVC.delegate = self;
    indentReportVC.reportTag = 1;
    indentReportVC.reportName = @"订单上报";
    [self.navigationController pushViewController:indentReportVC animated:NO];
    [indentReportVC release];
}
...
//    NSLog(@"%@",[aContentDic objectForKey:@"cause"]);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        ...
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView indentRecordCellViewWithArray:listArray]];
