//
//  AKDailyRecordContentViewController.m
//  农化通
//
...
@interface AKDailyRecordContentViewController ()
...
@implementation AKDailyRecordContentViewController
...
- (void)addQueryView
{
    ...
    UILabel *topLabel = [AKFastLabel labelWithFrame:CGRectMake(50, 0, 280, 44) andText:@"日报查询：" andtextAlignment:UITextAlignmentLeft andFont:22];
    topLabel.textColor = [UIColor whiteColor];
    UILabel *queryLabel1 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 50, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"日报标题：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel1.textColor = [UIColor whiteColor];
    UILabel *queryLabel2 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 65+_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"日报内容：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel2.textColor = [UIColor whiteColor];
    UILabel *queryLabel3 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 80+2*_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"开始时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel3.textColor = [UIColor whiteColor];
    UILabel *queryLabel4 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 95+3*_LINE_HEIGHT_, _FOUR_WIDTH_, _LINE_HEIGHT_) andText:@"结束时间：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    ...
    queryTextField3.placeholder = @"格式：2013-1-1";
    queryTextField3.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    ...
    queryTextField4.placeholder = @"格式：2013-1-30";
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"日报记录" andtextAlignment:UITextAlignmentCenter andFont:18];
...
{
    mum.hidden = NO;
    NSDictionary *aParameterDic = [NSDictionary dictionaryWithObjectsAndKeys:userData.userId,@"cid",userData.userAccount,@"mobile",userData.userPassword,@"pwd",@"list",@"action",@"",@"pagenum",@"",@"title",@"",@"content",@"",@"sdate",@"",@"edate", nil];
    [parameterDic setDictionary:aParameterDic];
    [currentRequestContent currentRequestWithURL:_RIBAO_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
-(void)reportData//上报
{
    AKDailyReportViewController *dailyReportVC = [[AKDailyReportViewController alloc]init];
    dailyReportVC.delegate = self;
    [self.navigationController pushViewController:dailyReportVC animated:NO];
    [dailyReportVC release];
}
...
{
    [self touchesBegan:nil withEvent:nil];
    [parameterDic setObject:queryTextField1.text forKey:@"title"];
    [parameterDic setObject:queryTextField2.text forKey:@"content"];
    [parameterDic setObject:queryTextField3.text forKey:@"sdate"];
    [parameterDic setObject:queryTextField4.text forKey:@"edate"];
...
-(void)refreshSucceedWithContentDictionary:(NSDictionary*)aContentDic
{
    ...
    if ([flagStr isEqualToString:@"success"]) {
        NSArray *listArray = [aContentDic objectForKey:@"daysreport"];
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView dailyRecordCellViewWithArray:listArray]];
