//
//  AKAdjustCargoRecordContentViewController.m
//  农化通
//
...
@interface AKAdjustCargoRecordContentViewController ()
-(void)ransmitRequest;//发送请求
-(void)ransmitRequestWithDic:(NSDictionary*)aDic;//发送请求
-(void)refreshContent;//刷新
@end

@implementation AKAdjustCargoRecordContentViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"调货记录" andtextAlignment:UITextAlignmentCenter andFont:18];
...
-(void)ransmitRequest//发送请求
{
    ...
    [currentRequestContent currentRequestWithURL:_TIAOHUO_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
-(void)reportData//上报
{
    AKAdjustCargoRecordViewController *adjustCargoRecordVC = [[AKAdjustCargoRecordViewController alloc]init];
    adjustCargoRecordVC.delegate = self;
    [self.navigationController pushViewController:adjustCargoRecordVC animated:NO];
    [adjustCargoRecordVC release];
}
...
//    NSLog(@"%@",aContentDic);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        ...
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView adjustCargoRecordCellViewWithArray:listArray]];
